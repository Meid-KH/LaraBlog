    ...
    <article class="max-w-4xl mx-auto lg:grid lg:grid-cols-12 gap-x-10">
      ...
      <div class="col-span-8">
        <div class="hidden lg:flex justify-between mb-6">
          ...
          <div class="space-x-2">
            <a href="{{ route('post', $post->category->slug) }}"
        ...
        <div class="space-y-6">
          <div class="bg-white border-2 border-gray-200 rounded-xl p-5">
            <div class="flex items-center space-x-4">
              <img class="border-2 border-blue-300 flex-shring-0 rounded-full rounded-xl"
                src="https://i.pravatar.cc/50/{{ auth()->id() }}" alt="Avatar">
            ...
            @auth
              <form class="mt-5" method="POST" action="{{ route('store_comment', $post->slug) }} ">
                @csrf
                <textarea cols="30" name="body" rows="5" placeholder="Add comment..."
                  class="border-b-2 focus:border-blue-400 font-semibold outline-none py-2 text-sm w-full"></textarea>
                ...
                <div class="border-gray-200 flex justify-end mt-4 mb-1">
                  <button
                    class="bg-blue-500 font-semibold hover:bg-blue-600 px-10 py-3 rounded-3xl select-none text-white tracking-wider">Publish</button>
                </div>
              </form>
            @else
